...
struct NIOUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    ...
    mutating func decodeIfPresent(_ type: Bool.Type) throws -> Bool? {
        let value = self.source[self.currentIndex]
        return try self.handle.decodeIfPresent(value: value, type: type, forKey: NIOCodableKey(arrayIndex: self.currentIndex))
    }
    ...
    mutating func decodeIfPresent<T>(_ type: T.Type) throws -> T? where T : Decodable {
        ...
        if type == Bool?.self {
            return try self.decodeIfPresent(Bool?.self) as? T
        } else if type == Int?.self {
            return try self.decodeIfPresent(Int?.self) as? T
        } else if type == Int8?.self {
            return try self.decodeIfPresent(Int8?.self) as? T
        } else if type == Int16?.self {
            return try self.decodeIfPresent(Int16?.self) as? T
        } else if type == Int32?.self {
            return try self.decodeIfPresent(Int32?.self) as? T
        } else if type == Int64?.self {
            return try self.decodeIfPresent(Int64?.self) as? T
        } else if type == UInt?.self {
            return try self.decodeIfPresent(UInt?.self) as? T
        } else if type == UInt8?.self {
            return try self.decodeIfPresent(UInt8?.self) as? T
        } else if type == UInt16?.self {
            return try self.decodeIfPresent(UInt16?.self) as? T
        } else if type == UInt32?.self {
            return try self.decodeIfPresent(UInt32?.self) as? T
        } else if type == UInt64?.self {
            return try self.decodeIfPresent(UInt64?.self) as? T
        } else if type == Float?.self {
            return try self.decodeIfPresent(Float?.self) as? T
        } else if type == Double?.self {
            return try self.decodeIfPresent(Double?.self) as? T
        } else if type == String?.self {
            return try self.decodeIfPresent(String?.self) as? T
        }
